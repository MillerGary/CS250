...
public class LazyPrimMST {
    private double weight1;       // total weight of MST
    ...
    private boolean check(EdgeWeightedGraph G) {
        ...
        for (Edge e : edges()) {
            totalWeight1 += e.weight1();
        }
        for (Edge e : edges()) {
            totalWeight2 += e.weight2();
        }
        double EPSILON = 1E-12;
        if (Math.abs(totalWeight1 - weight1()) > EPSILON) {
            System.err.printf("Weight1 of edges does not equal weight1(): %f vs. %f\n", totalWeight1, weight1());
            return false;
        }
        if (Math.abs(totalWeight2 - weight2()) > EPSILON) {
            System.err.printf("Weight2 of edges does not equal weight2(): %f vs. %f\n", totalWeight2, weight2());
            return false;
        }
        ...
        for (Edge e : edges()) {
            ...
            for (Edge f : G.edges()) {
                int x = f.either(), y = f.other(x);
                if (!uf.connected(x, y)) {
                    if (f.weight1() < e.weight1()) {
                        System.err.println("Edge " + f + " violates cut optimality conditions");
                        return false;
                    }
                    if (f.weight2() < e.weight2()) {
