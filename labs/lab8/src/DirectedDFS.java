...
public class DirectedDFS {
     ...
     */
    public DirectedDFS(EdgeWeightedDigraph G, int s, double budget, ArrayList<String> cities) {
        marked = new boolean[G.V()];
        dfs(G, s, budget, cities);
    }
     ...
     */
    public DirectedDFS(EdgeWeightedDigraph G, Iterable<Integer> sources, double budget, ArrayList<String> cities) {
        marked = new boolean[G.V()];
        for (int v : sources) {
            if (!marked[v]) dfs(G, v, budget, cities);
        }
    }

    private void dfs(EdgeWeightedDigraph G, int v, double budget, ArrayList<String> cities) {
        count++;
        marked[v] = true;
        //for (int i = 0; i < G.outdegree(v); i++) {
            //System.out.println("current v is " +cities.get(v)+ " has an outdegree of " +G.outdegree(v));
            for (int w : G.adjInt(v)) {
                if (!marked[w] /*&& (budget - w.weight2()) > 0*/) {
                    //budget = budget - w.weight2();
                    System.out.println(cities.get(v)+ " can reach " +cities.get(w)/*+ ", this leg costs $" +w.weight2()+ ", remaining budget is $" +budget*/);
                    dfs(G, w, budget, cities);
                } //if not marked
                /*if (marked[i]) {
                    //budget = budget - w.weight2();
                    //System.out.println("Source is " +cities.get(v)+ " and can reach " +cities.get(i)+ ", this leg costs $" +w.weight2()+ ", remaining budget is $" +budget);
                } //if marked output and subtract from budget*/
            } //inner-for
        //} //outter-for
    }
    ...
    /*public static void printDFS(EdgeWeightedDigraph G, ArrayList<String> cities) {
        for (int v = 0; v < G.V(); v++) {
