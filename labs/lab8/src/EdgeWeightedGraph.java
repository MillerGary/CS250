...
public class EdgeWeightedGraph {
    ...
    //method used to find all affordable trips within a budget constraint
    public Iterable<String> budgetTrip (double budget, ArrayList<String> cities) {
        ...
        for (int i = 0; i < V; i++) {
            ...
            for (Edge e : adj(i)) {
                if (e.other(i) > i) {
                    str = "\n" +cities.get(e.getV())+ "-" +cities.get(e.getW())+ ", this leg cost $" +e.weight2();
                    trips.push(str);
                } //if
                //only add one copy of each self loop (self loops will be consecutive)
                else if (e.other(i) == i) {
                    if (selfLoops % 2 == 0) {
                        str = "\n" +cities.get(e.getV())+ "-" +cities.get(e.getW())+ ", this leg cost $" +e.weight2();
                        trips.push(str);
                    } //push single copy to Stack
                    //increment the selfLoop counter
                    selfLoops++;
                } //else if i is a copy
            } //inner-for
        } //outter-for<`0`>
    ...
    public String printGraph(ArrayList<String> cities) {
        String NEWLINE = System.getProperty("line.separator");
        StringBuilder s = new StringBuilder();
        s.append(V + " " + E + NEWLINE);
        for (int v = 0; v < V; v++) {
            s.append("Flights Leaving or Arriving in " +cities.get(v)+ ": ");
            s.append(NEWLINE);
            for (Edge e : adj[v]) {
                s.append(e.toString(cities)+ "  ");
                s.append(NEWLINE);
            } //for
            s.append(NEWLINE);
        }
